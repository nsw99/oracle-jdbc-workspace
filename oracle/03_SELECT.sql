/*
 ...
 - 관계형(Relational) 데이터베이스
    ...
    3. 데이터의 중복을 최소화 할 수 있으며 사용하기 편리하고 데이터의 무결성, 트랜잭션 처리 등 데이터
        베이스 관리 시스템으로 뛰어난 성능을 보여준다.
    ...
    - SQL 종류
        ...
        2. DML (Data Manipulation Language) :  데이터 조작어
            ...
            ( INSERT 삽입, UPDATE수정 , DELETE 삭제) - 데이터
        ...
        5. TCL (Trancsaction Control Language) : 트랜잭션을 제어하는 언어    
...
SELECT EMAIL,ENT_DATE, EMP_NO, EMP_NAME
FROM EMPLOYEE;

--5. EMPLOYEE 테이블의 입사일, 직원명, 급여 조회
SELECT HIRE_DATE, SALARY, EMP_NAME
FROM EMPLOYEE;


/*
    컬럼의 산술 연산
    - SELECT 절에 컬럼명 입력 부분에서 산술 연산자를 사용하여 결과를 조회 할 수 있다.
*/
-- EMPLOYEE 테이블에서 직원명 (EMP_NAME), 직원의 연봉 조회(급여 SALARY*12)
...
/*
    ...
    컬럼 AS 별칭 / 컬럼 AS "별청" / 컬럼 별칭 / 컬럼 "별칭"
   
    - 산술연산을 하게 되면 컬럼명이 지저분해진다. 이떄 컬럼명에 별칭을 부여해서 깔끔하게 보여줄 수 있다.
...
SELECT EMP_NAME || '의 월급은' || SALARY || '원' AS "급여정보"
...
-- 유의사항 ! DISTINCT는 SELCET절에 딱 한번만 기술 가능
...
SELECT EMP_NAME " 사원명 ", DEPT_CODE " 부서코드 ",SALARY " 급여조회 "
FROM EMPLOYEE
WHERE SALARY>=4000000;

-- 재직중(ENT_YN 컬럼값이 'N')인 사원들의 사번, 사원명, 입사일 조회
SELECT EMP_ID " 사번 ", EMP_NAME "사원명 ", HIRE_DATE " 입사일 ", ENT_YN " 재직중 "  
FROM EMPLOYEE 
WHERE ENT_YN ='N';
...
-- OR(또는), AND (그리고)
-- 부서코드가 'D6'이거나 급여가 300만원 이상인 사람들의 사원명,부서코드,급여조회
SELECT EMP_NAME,DEPT_CODE,SALARY||'원' AS "월급" 
FROM EMPLOYEE
...
/*
Like
- 비교 하고자 하는 컬럼값이 내가 제시한 특정 패턴에 만족될 경우 조회
...
/*
    IN
    - 비교대상컬럼값이 내가 제시한 목록 중에 일치한 값이 있는지
...
SELECT EMP_NAME NAME , DEPT_CODE CODE , SALARY
FROM EMPLOYEE
...
SELECT  MANAGER_ID 부서코드, EMP_NAME,DEPT_CODE
FROM EMPLOYEE
WHERE MANAGER_ID IS  NULL AND DEPT_CODE IS NULL;

-- 2. 연봉(보너스 포함X)이 3000만원 이상이고 보너스를 받지 않은 사원들의
--사번,사원명,급여,보너스 조회
...
--3. 입사일 '95/01/01' 이상이고 부서배치를 받은 사원들의 사번,사원명,입사일,부서코드 조회
...
/*
    ...
    - ASC : 오름차순 정렬(생략해도 기본값으로)
    - DESC : 내림차순 정령
    - NULLS FIRST : 정렬하고자 하는 컬럼 값에 NULL이 있을 경우 해당 데이터를 맨 앞 배치 (생략시  DESC일때 기본값)
    - NULLS LAST : 정렬하고자 하는 컬럼 값에 NULL이 있을 경우 해당 데이터를 맨 뒤 배치 (생략시 ASC일때 기본값)
