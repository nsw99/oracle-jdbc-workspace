...
-- EMP_DEPT 테이블에 EMPLOYEE 테이블의 부서코드가 D1인 직원의 사번
-- 이름, 부서코드 입사일을 삽입하고
-- EMP_MANAGER 테이블에 EMPLOYEE 테이블의 부서코드가 D1인 직원의 사번
...
AS SELECT EMP_ID,EMP_NAME,HIRE_DATE,SALARY FROM EMPLOYEE
WHERE 0=1;
...
-- EMPLOYEE 테이블 입사일 기준으로 2000년 1월 1일 이전에
-- 입사한 사원 정보는 EMP_OLD 테이블에 삽입하고
-- 2000년 1월 1일 이후에 입사한 사원의 정보는 EMP_NEW 테이블에 삽입한다.

INSERT ALL
WHEN HIRE_DATE < '2000/01/1'
...
/*
    ...
    SET 컬럼명 = 변경하려는 값, 
        ...
        - UPDATE 시 서브쿼리를 사용해서 서브쿼리를 수행한 결과값으로 컬럼 값을
        변경할 수 있다.
*/
...
ROLLBACK;
-- DEPT_COPY 테이블에서 DEPT_ID가 'D9' 인 부서명을 전략기회팀 으로 수정
UPDATE DEPT_COPY
...
--EMP_SALARY 테이블에서 선동일 사장의 급여를 7000원으로, 보너스를 0.2로변경
UPDATE EMP_SALARY
SET  SALARY = 7000
SET BONUS = 0.2
WHERE EMP_NAME = '선동일';
...
FROM EMPLOYEE WHERE EMP_NAME='유재식')
, BONUS = (SELECT BONUS
FROM EMPLOYEE WHERE EMP_NAME='유재식')
WHERE EMP_NAME = '방명수';
...
-- 다중열 서브쿼리
UPDATE EMP_SALARY
SET (SALARY,BONUS)=(SELECT SALARY,BONUS FROM EMPLOYEE WHERE 
EMP_NAME = '유재식')
WHERE EMP_NAME = '방명수';


-- EMP_SALARY 테이블에서 아시아 지역에 근무하는
--직원들의 보너스를 0.3으로 변경
...
-- EMP_SALARY 테이블에서 DEPT_CODE가 5인 직원들을 삭제
DELETE FROM EMP_SALARY
WHERE DEPT_CODE = 'D5';
