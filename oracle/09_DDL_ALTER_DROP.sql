 ...
 ALTER TABLE DEPT_COPY ADD CNAME VARCHAR2(20);
  ...
  /*
    1-2) 컬럼 수정 (MODIFY)
     ...
     - 기본값 변경 : ALTER TABLE 테이블명 MODFIY 컬럼명 DEFAULT 변경할 기본값;       
                            --> DEFAULT NULL : DEFAULT 삭제
  */
  
  -- DEPT_ID 컬럼의 데이터 타입을 CAHR(3)으로 변경
...
/*
    ...
    FOREIGN KEY : ADD [CONSTRAINT 제약조건명] FOREIGN KEY(컬럼명) REFRENCES 테이블명 [(컬럼명)];
...
/*
    ...
    3_2) 제약 조건명 변경
    ALTER TABLE 테이블명 RENAME CONSTRAINT 기존 제약조건명 
    TO 변경할 제약 조건명;
*/
...
/*
        3-3) 테이블명 변경
                ALTER TABLE 기존 테이블명 RENAME TO 변경할 테이블먕;
...
--만약 삭제하고자 한다면 
--1. 자식 테이블을 먼제 삭제후 부모테이블을 삭제
--2. 그냥 부모테이블만 삭제하는데 제약조건까지 같이 삭제하는 방법
DROP TABLE MEMEBER;
DROP TABLE MEMBER_GRADE;
