...
SELECT NVL(DEPT_CODE,'부서없음')AS"부서",COUNT(EMP_NAME), COUNT(BONUS), SUM(SALARY), ROUND(AVG(SALARY)), MIN(SALARY),MAX(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE
ORDER BY DEPT_CODE NULLS FIRST ;

-- 성별 별(남자/여자) 사원의 수 조회
SELECT DEPT_CODE, COUNT(DECODE(SUBSTR(EMP_NO,8,1),1 ,'남',2,'여'))AS"사원의 수"
FROM EMPLOYEE
GROUP BY DEPT_CODE 
ORDER BY 1;
...
-- 컬럼이 하나일 때는 사실상 CUBE 와 ROLLUP이 차이점이 딱히 없음!!

-- 부서 코드와 직급 코드가 같은 사원들의 급여 합계 조히
...
/*
    ...
    MINUS(차집합) : 선행 쿼리문의 결과값에서 후행 쿼리문의 결과값을 뺀 나머지
                         결과 값만 추출 
*/
-- 부서 코드가 D5인 사원들이 사번,사원명, 부서코드, 급여를 조회
...
-- 3. INTERSECT
-- 부서코드가D5이면서 급여가 300만원 초과인 사원 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
WHERE DEPT_CODE='D5'
INTERSECT
