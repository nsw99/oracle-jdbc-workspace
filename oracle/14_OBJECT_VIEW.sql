...
/*
    ...
    * VIEW 옵션들
    - OR REPLACE : 뷰 생성시 기존에 중복된 이름의 뷰가 없다면
                            새로이 뷰를 생성하고, 기존에 중복된 이름의 뷰가 있다면
                            해당 뷰를 변경(갱신)하는 옵션 
...
-- 사원의 사번, 사원명, 직급명,성별,근무년수를 조회

SELECT EMP_ID,EMP_NAME,JOB_NAME, DECODE(SUBSTR(EMP_NO,8,1),1,'남',2,'여') AS "성별",ROUND((SYSDATE -HIRE_DATE)/365)
FROM EMPLOYEE
...
/*
    ...
    VIEW 를 이용해서 DML(UPDATE,INSERT,DELETE) 사용 가능
    - 뷰를 통해서 조작하게 되면 실제 데이터가 담겨 있는
    베이스 테이블에 반영됨
*/
...
SELECT * FROM  VW_JOB; --논리적 테이블(실제데이터가 담겨있진 않음)
 ...
 */
 ...
 --2. 뷰에 포함되지 않은 컬럼중에 베이스가 되는 컬럼이 NOT NULL제약조건이
 --지정된 경우
 CREATE OR REPLACE VIEW VW_JOB
 AS SELECT JOB_NAME
 FROM JOB;
 ...
 UPDATE VW_EMP_SAL
 SET 연봉 = 80000000 -- 에러 (산술연산으로 정의된 컬럼은 데이터 변경 불가능)
 WHERE EMP_ID = 200;
 ...
 SELECT * FROM VW_EMP_SAL; -- 성공 (산술연산과 무관한 컬럼은 데이터변경 가능)
 ...
 SELECT DEPT_TITLE,SUM(SALARY), FLOOR(AVG(NVL(SALARY,0)))
 FROM EMPLOYEE
 JOIN DEPARTMENT ON (DEPT_ID=DEPT_CODE)
 GROUP BY DEPT_TITLE;
...
INSERT INTO VW_JOINEMP(EMP_ID,EMP_NAME)VALUES(300,'조세오');
...
--VIEW 옵션
--2. FORCE | NOFORCE
-- NOFROCE (기본값)  : 서브쿼리에 기술된 테이블이 존재하는 테이블이여만
--                                  뷰가 생성되게 함
DROP TABLE TEST;
DROP VIEW TEST;

-- 안되는게 정상 ! 테이블이 있어야만 만들수 있음
...
UPDATE VW_EMP
SET SALARY =2000000 --> 서브쿼리에 기술한 조건에 부합되지 않기 떄문에
WHERE EMP_ID = 200;

UPDATE VW_EMP
SET SALARY =4000000
WHERE EMP_ID =200; --> 서브쿼리에 기술한 조건에 부합되지 않기 때문에 변경 가능
