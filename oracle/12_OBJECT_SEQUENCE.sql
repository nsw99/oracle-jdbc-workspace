...
/*
    ...
    [START WITH 시작숫자] --> 처음 발생시킬 시작값을 지정 (기본값은 1)
    [INCREMENT BY 숫자] --> 몇 씩 증가시킬 건지 (기본값은 1)
    [MAXVALUE 숫자] --> 최대값 지정 (기본값이 큼)
    [MINVALUE 숫자] --> 최소값 지정 (기본값은 1)
    [CYCLE | NOCYCLE] --> 값 순환 여부 지정 (기본값은 NOCYCLE)
    [NOCACHE | CACHE 바이트크기] --> 캐시메모르 할당 (기본값은 CACHE 20)
...
-- 현재 계정이 소유하고 있는 시퀀스들을 구조를 보고자 할 때
...
-- 2) 매번 새로운 사번이 발생되는 SEQUENCE 사용해서 INSERT 구문 사용
INSERT INTO EMPLOYEE(EMP_ID,EMP_NAME,EMP_NO)
VALUES (SEQ_EMPID.NEXTVAL,'장밥두','961234_1234564');

INSERT INTO EMPLOYEE(EMP_ID,EMP_NAME,EMP_NO)
VALUES (SEQ_EMPID.NEXTVAL,'고아라','230629-487512'); 
