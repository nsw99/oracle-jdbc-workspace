...
/*
    DEFAULT 기본값 
    - 제약조건 아님!
    - 컬럼을 선정하지 않고 INSERT시 NULL이 아닌 기본값을 INSERT하고자 할 때 세팅해둘
    수 있는 값
    [표현식]
...
INSERT INTO MEMBER VALUES(1,'홍길순',NULL,NULL,NULL);
...
INSERT INTO TB_MEMBER VALUES(3,'USER3','1234','강호동','F','서울시 강남구','010-5555-2222','N','23/06/27');
...
RENT_MEM_NO NUMBER REFERENCES TB_MEMBER ON DELETE CASCADE,
RENT_BOOK_NO NUMBER REFERENCES TB_BOOK ON DELETE CASCADE,
...
WHERE RENT_BOOK_NO IN '2';


SELECT BK_TITLE, PUB_NAME, RENT_DATE, RENT_DATE+7
FROM TB_RENT
JOIN TB_BOOK ON (BK_PUBNO = RENT_NO)
JOIN  TB_PUBLISHER ON (PUB_NO = RENT_NO)
JOIN TB_MEMBER ON (MEMBER_NO = RENT_BOOK_NO)
WHERE MEMBER_NO ='1';